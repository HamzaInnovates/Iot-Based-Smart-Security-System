...
#Constructor 
    def __init__(self,root):
        ...
        self.root =root
        self.root.geometry("400x300+500+220")
        ...
        self.reset_lbl = customtkinter.CTkLabel(self.main_frame,text="Reset Password",font=font)
        self.reset_lbl.place(relx=0.3,rely=0.1)
        self.reset_frame = customtkinter.CTkFrame(self.main_frame,border_width=1,border_color="grey",width=334,height=200,fg_color="transparent")
        self.reset_frame.place(relx=0.06,rely=0.3)
        self.switch = customtkinter.CTkSwitch(self.reset_frame,font=inner_font,text="Switch App Mode",command=self.change_color,variable=switch_1,onvalue="on",offvalue="off")
        self.switch.place(x=10,rely=0.85)
        self.username=customtkinter.CTkEntry(self.reset_frame,font=inner_font,placeholder_text="Enter Your UserName")
        self.username.place(x=95,y=10)
        self.sec_ques = customtkinter.CTkComboBox(self.reset_frame,font=inner_font,values=["Choose Question","Friend's Name","Pet's Name","Favourite Hobby","Date of Birth"])
        self.sec_ques.place(x=10,y=50)
        self.sec_ans = customtkinter.CTkEntry(self.reset_frame,placeholder_text="Enter Security Answer",font=inner_font)
        self.sec_ans.place(x=180,y=50)
        self.password_new = customtkinter.CTkEntry(self.reset_frame,placeholder_text="Enter New Password",font=inner_font,show="*")
        self.password_new.place(x=10,y=90)
        self.confirm_password = customtkinter.CTkEntry(self.reset_frame,placeholder_text="Confirm Password",font=inner_font,show="*")
        self.confirm_password.place(x=180,y=90)
        self.reset_btn = customtkinter.CTkButton(self.reset_frame,text="Reset",font=inner_font,fg_color="green",command=self.reset_password)
        self.reset_btn.place(x=10,y=134)
        self.reset_btn = customtkinter.CTkButton(self.reset_frame,text="Clear",font=inner_font,fg_color="Red",command=self.reset)
...
# reset the fields
    def reset(self): 
        ...
        self.sec_ques.set("Choose Question"),
        self.sec_ans.delete(0,"end"),
        self.password_new.delete(0,"end"),
        self.confirm_password.delete(0,"end"),
        self.main_frame.focus() 
# reset the password
    def reset_password(self):
        if self.username.get()==""or self.sec_ques.get()==""or self.sec_ans.get=="" or self.password_new.get()==""or self.confirm_password.get()=="":
            messagebox.showerror("Error","All Fields are required!",parent=self.root)
        elif self.sec_ques.get()=="Choose Question" :
            messagebox.showerror("Error","PLease Select the Security Question",parent=self.root)   
        else:    
            conn= mysql.connector.connect(host= 'localhost',user='root', password='@fyp2020',database="design")
            cursor =conn.cursor()
            cursor.execute("SELECT Security_Ques,Security_Ans from admin where Email = %s",(self.username.get(),))
            data = cursor.fetchone()
            if data is not None:
                ques=data[0]
                ans =data[1]  
                if  self.sec_ques.get()==ques and self.sec_ans.get().lower()==ans.lower():
                    if not re.match(pattern_password,self.password_new.get()):
                        ...
                        return False
                    elif self.password_new.get()!=self.confirm_password.get():
                        messagebox.showwarning('Password Mismatch', 'The password does not match.\n Please try again.',parent=self.root)
                        return False
                    else:
                        cursor.execute("UPDATE admin set Password =%s where Email =%s",(self.password_new.get(),self.username.get(),))
                        conn.commit()
                        messagebox.showinfo("Sucess","Your Password has been Reset. Please Login Again.",parent=self.root)
                        self.open_login_screen()      
                else:
                    messagebox.showerror("Error","Wrong Security Question or Answer",parent=self.root)  
            else:
        ...
        with open(r"Attendance.csv", "r+", newline="\n") as f:
            ...
            if str(emp_id) not in name_list:
                ...
                if last_log_date == date:
                    ...
                    self.voice_gen()
                    self.rotate()
# Thread for the voice
    def voice(self):
        voice_thread=threading.Thread(target=self.run_voice)
        voice_thread.start()
# To run the voice while automating the door  
    def run_voice(self):
        engine = pyttsx3.init()
        voices=engine.getProperty('voices')
        engine.setProperty('voice',voices[1].id)
...
# Stores the access
    def store_Access(self, emp_id, f_name, Dep):
        ...
        if last_date is not None and last_time is not None:
            ...
            conn.commit()
            self.rotate()               
# To give the access to only authorized employee
